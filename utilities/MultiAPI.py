...
from langchain_anthropic import ChatAnthropic  # Added Anthropic support
from langchain_google_genai import ChatGoogleGenerativeAI  # Added Google Gemini support
from langchain_community.chat_models import ChatFireworks  # Added Fireworks AI support
from langchain_mistralai import ChatMistralAI # Added Mistral support
...
# New function to get LLM based on provider selection
def get_llm(provider, model, api_key=None, api_key_2=None):
    """Create and return the selected LLM"""
    if provider == "OpenAI":
        return ChatOpenAI(
            temperature=0, 
            model=model,
...
def main():
    ...
    # LLM settings - updated to support multiple providers
    st.sidebar.header("LLM Provider Settings")
    
    # Provider selection
    provider_options = ["OpenAI", "Anthropic", "Google", "Fireworks", "Mistral"]
    ...
    model_options = {
        ...
        "Fireworks": {
            "Qwen 1.5 72B": "accounts/fireworks/models/qwen1.5-72b-chat",
            "Qwen 1.5 32B": "accounts/fireworks/models/qwen1.5-32b-chat",
            "Qwen 1.5 14B": "accounts/fireworks/models/qwen1.5-14b-chat",
            "Qwen 1.5 7B": "accounts/fireworks/models/qwen1.5-7b-chat",
    ...
    selected_model = st.sidebar.selectbox(
        ...
        list(model_options[selected_provider].keys())
    )
    ...
    if st.button("Generate Knowledge Graph"):
        if not api_key:
            ...
            if not content:
                ...
                # Initialize LangChain components with the selected provider
                llm = get_llm(
                    selected_provider, 
                    model_options[selected_provider][selected_model],
                    api_key
                )
                
                if llm is None:
                    st.error(f"Failed to initialize {selected_provider} LLM. Please check your API key and selected model.")
                    return
                ...
                if nodes and relationships:
                    ...
                    st.error("No entities or relationships were extracted. Try adjusting chunk size or using a different model.")
